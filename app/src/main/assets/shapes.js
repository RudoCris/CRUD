function drawEtalon(e){
  var etalon = e.split('\n');
  etalon.pop();
  for(var i=0; i<etalon.length; i++){
    etalon[i] = etalon[i].match(/(\d|-\d)\S*/gi);
  }
  var min = etalon[0][2], max = etalon[0][2];
  
  for (var i = 1; i < etalon.length; i++) {
    min = Math.min(etalon[i][2],min);
    max = Math.max(etalon[i][2],max);
  };
